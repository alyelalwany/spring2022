...
procedure endterm_3 is
   ...
   protected type OperaHouse (id : Integer_access) is
   ...
   begin
      ...
      while not bought and nr_of_tries < 3 loop
         Printer.print
           ("Person with id " & Positive'Image (id.all) & " at " &
            from_1_to_2'Image (rand_opera) & " : " & Boolean'Image (bought) &
            " show = " & from_1_to_2'Image (rand_show));
         select
...
begin
   ...
   for I in operas'Range loop
      operas (I) := new OperaHouse (new Integer'(I));
   ...
   for I in 1 .. 3 loop
      Printer.print
        ("We sold total of " & Natural'Image (operas (I).get_count_tickets) &
         " tickets and served "& Natural'Image (operas(I).get_count_people_served) &" people ");
   end loop;
