...
procedure endterm_5 is
   ...
   protected body OperaHouse is
      entry buying (nr_of_tickets : Natural; opera_to_attend : from_1_to_2)
        when max_capacity.all > total_people_served is
      begin
   ...
   task covid_test is
      entry check_for_covid (has_covid : out Boolean; Id : Integer);
   end covid_test;
   task body covid_test is
      counter : Natural := 0;
   begin
      while counter < 40 loop
         select
            accept check_for_covid (has_covid : out Boolean; Id : Integer) do
               counter := counter + 1;
               if Id > 0 and Id mod 5 = 0 then
                  has_covid := True;
               else
                  has_covid := False;
               end if;
   ...
   begin
      ...
      while not bought and nr_of_tries < 3 loop
         select
            covid_test.check_for_covid (has_covid, id.all);
            if not has_covid then
